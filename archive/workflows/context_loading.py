...
def _stage_retrieve_documents(task_input: str, meta: Dict[str, Any]) -> List[AgentAction]:
    ...
    if isinstance(request_plan, dict) and request_plan.get("mode") == "compound_plan":
        query = (
            ...
            f"Needed context sources: {', '.join(request_plan.get('needed_context_sources', []))}"
        )
    return [
...
_EMAIL_WORKFLOWS = frozenset({"email_watcher", "email_ingestion", "morning_brief", "weekly_recap"})
_CALENDAR_WORKFLOWS = frozenset({"calendar_briefing", "meeting_prep", "schedule_planning", "day_schedule_planning", "week_schedule_planning"})


def _stage_live_connector(task_input: str, meta: Dict[str, Any]) -> List[AgentAction]:
    workflow_type = meta.get("workflow_type", "")
    actions: List[AgentAction] = []
    if workflow_type in _EMAIL_WORKFLOWS or workflow_type not in _CALENDAR_WORKFLOWS:
        actions.append(tool_action("read_email_threads", result_key="live_threads", limit=10))
    if workflow_type in _CALENDAR_WORKFLOWS or workflow_type not in _EMAIL_WORKFLOWS:
...
def prepare_briefing_context(context: Dict[str, Any], attachments: List[Dict[str, Any]]) -> BriefingWorkflowContext:
    ...
    if live_threads:
        event_payload.setdefault("live_threads", live_threads)
    if live_events:
        event_payload.setdefault("live_events", live_events)
