...
# ---------------------------------------------------------------------------
# Metric taxonomy — built from canonical finance definitions at import time.
# Read by _infer_finance_category and _normalize_metric_name.
# ---------------------------------------------------------------------------
_METRIC_TAXONOMY: Dict[str, tuple] = {
    d.metric_key: (d.category, [d.metric_label.lower()])
...
class _ReportIntent:
    """Semantic intent for a single report request.

    Created once from the RequestPlan already computed upstream.
    All routing in ReportAgent reads from this struct — no string scanning.
    """
    is_finance: bool
    is_escalation: bool
    is_action_plan: bool
    ...
    @classmethod
    def from_plan(cls, plan: RequestPlan) -> "_ReportIntent":
        meta = plan.planning_metadata
        return cls(
            is_finance=bool(meta.get("mentions_finance_analysis")),
            is_escalation=bool(meta.get("mentions_escalation")),
            is_action_plan=bool(meta.get("mentions_action_plan")),
            is_recommendation=bool(meta.get("mentions_recommendation")),
        )

    @classmethod
    def from_workflow_metadata(cls, metadata: dict) -> "_ReportIntent":
        """Build from the serialized request_plan dict stored in workflow metadata."""
        plan_raw = metadata.get("request_plan") or {}
        planning_meta = plan_raw.get("planning_metadata") or {}
        return cls(
            is_finance=bool(planning_meta.get("mentions_finance_analysis")),
            is_escalation=bool(planning_meta.get("mentions_escalation")),
            is_action_plan=bool(planning_meta.get("mentions_action_plan")),
            is_recommendation=bool(planning_meta.get("mentions_recommendation")),
        )

    @classmethod
    def from_task_input(cls, task_input: str) -> "_ReportIntent":
        """Fallback: classify inline when no pre-computed plan is available."""
        return cls.from_plan(plan_request(task_input))


class ReportAgent(BaseAgent):
    COMPLETION_MODEL = os.getenv("REPORT_AGENT_MODEL", DEFAULT_ANTHROPIC_MODEL)

    SOURCE_TRUST_RANKS = SOURCE_TRUST_RANKS
    ...
    def __init__(self, tools: ToolRegistry):
        ...
        self._finance_template_cache: dict[str, Optional[str]] = {}
    ...
    async def run(self, agent_input: AgentInput, **kwargs: Any) -> AgentOutput:
        ...
        intent = _ReportIntent.from_workflow_metadata(agent_input.workflow_state.metadata or {})
        ...
        # Compute resolved topics from session history + current turn for reuse below.
        resolved_topics = self._extract_resolved_topics(session_history, task_input=task_input)
        # Structural clarification answers take precedence over keyword-inferred resolved_topics.
        resolved_clarifications: dict[str, str] = (
            agent_input.workflow_state.metadata.get("resolved_clarifications") or {}
        )
        ...
        pinned_modality: str | None = agent_input.workflow_state.metadata.get("pinned_output_modality")
        if pinned_modality:
            output_modality, artifact_plan = self._modality_to_artifact_plan(pinned_modality)
        else:
        ...
        primary_visual = get_finance_template_primary_visual(finance_template) if finance_template else None
    ...
    def _report_prompt(
        ...
        recommendation_block = self._recommendation_request_block(
            intent or _ReportIntent.from_task_input(task_input)
        )
        ...
        return self.load_prompt(
            "report_base",
            repeat_frustration_block=repeat_frustration_block,
            pending_questions_block=pending_questions_block,
            task_input=task_input,
            unified_memory_block=unified_memory_block,
            financial_task_block=financial_task_block,
            manifest_block=manifest_block,
            vocabulary_block=vocabulary_block,
            compact_company_state=compact_company_state,
            compact_company_identity=compact_company_identity,
            compact_preferences=compact_preferences,
            compact_project_context=compact_project_context,
            ...
            composition_plan_block="",  # Included in report_base.md
            kb_block=kb_block,
            recommendation_block=recommendation_block,
            metric_block=metric_block,
            schedule_block=schedule_block,
            followup_block=followup_block,
            live_context_followup_block=live_context_followup_block,
            obs_block=obs_block,
            artifact_block=artifact_block,
            email_scope_block=email_scope_block,
    ...
    def _artifact_type_instruction_block(
        ...
        if artifact_type == "board_brief":
            labels = get_finance_template_section_labels(finance_template) if finance_template else [
                "Executive Summary", "Key Findings", "Recommended Actions"
            ]
            return self.load_prompt("report_artifact_types", mode="BOARD BRIEF", labels=labels)
        if artifact_type == "action_plan":
            return self.load_prompt("report_artifact_types", mode="ACTION PLAN")
        if artifact_type == "email":
            return self.load_prompt("report_artifact_types", mode="EMAIL DRAFT")

        # Default "report" mode — use existing prompt instructions unchanged.
        preferred_labels = (
            get_finance_template_section_labels(finance_template) if finance_template
            else ["Key Finding", "Business Implications", "Recommended Actions"]
        )
        return self.load_prompt("report_artifact_types", mode="DEFAULT REPORT", preferred_labels=preferred_labels)
    ...
    def _recommendation_request_block(self, intent: "_ReportIntent") -> str:
        if not intent.is_recommendation:
            return ""
        return (
    ...
    def _build_artifact_actions(
        ...
        *,
        ...
        if output_modality in {"docx", "docx+xlsx"}:
            ...
            theme_id = (
                get_finance_template_definition_resolved(finance_template).default_theme_id
                if finance_template
    ...
    def _select_finance_template(self, task_input: str) -> Optional[str]:
        key = (task_input or "").strip()
        if not key:
            return None
        if key in self._finance_template_cache:
            return self._finance_template_cache[key]

        options = get_finance_template_request_options()
        prompt = (
            "Classify whether the user request is asking for a finance workbook/report template.\n"
            "Return strict JSON with keys: template, confidence.\n"
            "template must be one of: " + ", ".join(options) + ", none.\n"
            "If not clearly a finance request, return template='none'.\n\n"
            f"User request:\n{task_input}"
        )
        selected: Optional[str] = None
        try:
            raw = LLMClient(model=self.COMPLETION_MODEL).complete(prompt)
            parsed = json.loads(raw)
            candidate = str(parsed.get("template") or "").strip()
            if candidate in options:
                selected = candidate
            elif candidate in {"none", "null", ""}:
                selected = None
        except Exception:
            selected = None

        self._finance_template_cache[key] = selected
        return selected
    ...
    def _clarifying_questions(
        ...
        # Routing reads from the pre-computed intent struct — no keyword scanning here.
        if intent and intent.is_finance:
            add("Frame this for your own decision or for the board?")
        elif intent and intent.is_escalation:
            add("Choose one now: draft a ready-to-use response, or get a brief-only analysis?")
    ...
    def _clarification_options(self, *, task_input: str, questions: list[str], intent: Optional["_ReportIntent"] = None) -> list[dict[str, Any]]:
        """
        ...
        if "period" in question_text or (intent and intent.is_finance):
            return [
        ...
        if "draft" in question_text or (intent and intent.is_escalation):
            return [
    ...
    def _options_for_question(self, question: str, task_input: str, intent: Optional["_ReportIntent"] = None) -> List[Dict[str, Any]]:
        """Return 2-3 selectable answer options for a single open question."""
        return self._clarification_options(task_input=task_input, questions=[question], intent=intent)

    def _collect_trust_options(
        self,
        task_input: str,
        payload: ReportPayload,
        resolved_topics: frozenset,
        ...
        if "output_format" in resolved_topics:
            ...
            clarifying_questions = self._clarifying_questions(
                task_input, payload, ceo_id=ceo_id, resolved_topics=resolved_topics, intent=intent
            )
        for q in clarifying_questions:
            opts = self._options_for_question(q, task_input, intent=intent)
            diag.log_decision(
    ...
    def _build_finance_workbook_spec(
        ...
        *,
        ...
        return WorkbookSpec(
            ...
            metadata={
                "template": finance_template,
                "template_id": get_finance_template_definition_resolved(finance_template).workbook_template_id,
                "theme_id": get_finance_template_definition_resolved(finance_template).default_theme_id,
                "presentation_version": "workbook_spec_v1",
                "expected_metrics": get_finance_template_expected_metrics(finance_template),
                "expected_periods": get_finance_template_expected_periods(finance_template),
    ...
    def _apply_finance_accuracy_guardrails(
        ...
        *,
        ...
        qa_result = run_finance_qa_checklist(
            rows=rows,
            expected_metric_keys=get_finance_template_expected_metrics(finance_template) if finance_template else [],
            company_state=company_state,
            finance_template=finance_template,
        )
    ...
    def _prepare_financial_rows(
        ...
        *,
        ...
        rows = self._sanitize_financial_rows(
            rows,
            aws_focus=(finance_template == "aws_cost_review"),
        )
    ...
    def _infer_finance_category(self, metric_label: str) -> str:
        ...
        if normalized != metric_label:
            for canonical, (category, _) in _METRIC_TAXONOMY.items():
    ...
    def _normalize_metric_name(self, metric_label: str, *, category: Optional[str] = None) -> str:
        ...
        for canonical, (canonical_category, aliases) in _METRIC_TAXONOMY.items():
    ...
    def _apply_presentation_metadata(
        ...
        *,
        ...
        # Structural clarification answers (from ConversationLiveContext) take precedence.
        # Augmenting resolved_topics here means all downstream gates inherit the answers
        # without any changes at individual call sites.
        if resolved_clarifications:
            resolved_topics = resolved_topics | frozenset(resolved_clarifications.keys())
