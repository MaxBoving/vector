...
class BriefingAgent(BaseAgent):
    ...
    async def run(self, agent_input: AgentInput, **kwargs: Any) -> AgentOutput:
        ...
        for _top_key in ("retrieval_manifest", "entity_context", "proactive_observations_block", "unified_memory"):
        ...
        if "briefing_completion" not in context and not self._has_live_event_context(workflow_type, event_payload):
            return AgentOutput(
                agent_name=self.metadata.name,
                stage=agent_input.stage,
                success=True,
        ...
        payload = self._apply_presentation_metadata(payload, event_payload=event_payload, workflow_type=workflow_type)
    ...
    def _generate_payload(
        ...
        *,
        ...
        return BriefPayload(
            ...
            trust=self._derive_trust(workflow_type, event_payload, prepared_context),
            sources=sources,
        )
    ...
    def _next_actions(self, workflow_type: str, ranked_threads: List[Dict[str, Any]]) -> List[str]:
        if workflow_type not in {"email_ingestion", "email_watcher"}:
            if workflow_type in {"schedule_planning"}:
                return [
                    "Confirm the first two must-do blocks before you start the day.",
                    "Use any linked meeting or doc prep items to tighten the final order.",
                    "Only convert a proposed block into a calendar event if you want it written back.",
                ]
            if workflow_type == "morning_brief":
                return [
                    "Review the top priority thread and next scheduled meeting first.",
                    "Confirm which asks need action this week versus next week.",
                    "Turn any board, investor, or prep requests into follow-up docs if needed.",
                ]
            if workflow_type == "weekly_recap":
                return [
                    "Identify any open items from this week that need to carry forward.",
                    "Send any outstanding replies or follow-ups surfaced in the recap.",
                    "Use the week's accomplishments to update stakeholders if needed.",
                ]
            if workflow_type == "meeting_prep":
                return [
                    "Review attendee threads before joining to know where each person stands.",
                    "Confirm any open asks or deliverables owed to meeting participants.",
                    "Prepare talking points grounded in the most recent correspondence.",
                ]
            if workflow_type == "calendar_briefing":
                return [
                    "Block prep time before any back-to-back meetings.",
                    "Check whether any meeting requires materials you haven't produced yet.",
                    "Flag any attendees who need a pre-read or agenda in advance.",
                ]
            return [
                "Review the highest-priority item first.",
                "Confirm whether any missing stakeholders need to be looped in.",
                "Use the supporting context before responding externally.",
            ]
        triage = self._email_triage_buckets(ranked_threads)
    ...
    def _default_title(
        ...
        if profile.title_kind == "morning_brief":
            if planning_context.get("target_label"):
                return f"{planning_context['target_label']} Watch Brief"
            return "Weekly Watch Brief"
    ...
    def _build_sections(
        ...
        *,
        ...
        if workflow_type in {"morning_brief"}:
            ...
            if not follow_ups:
                follow_ups = self._next_actions(workflow_type, ranked_threads)
            if workflow_type == "morning_brief":
                meetings = [
                    f"{event.get('title', 'Upcoming meeting')} • {event.get('starts_at', 'scheduled')}"
                    for event in self._filter_events_by_horizon(event_payload.get("upcoming_events", []) or [], event_payload)[:4]
                ] or meetings
            sections = [
        ...
        if workflow_type == "weekly_recap":
            ...
            open_items = asks[:3] or self._next_actions(workflow_type, ranked_threads)
            return [
        ...
        sections = [
            ...
            BriefSection(
                ...
                items=self._next_actions(workflow_type, ranked_threads),
            ),
        ]
    ...
    def _meeting_open_items(self, event_payload: Dict[str, Any], ranked_threads: List[Dict[str, Any]]) -> List[str]:
        structured_watch = event_payload.get("structured_watch", {}) or {}
        open_items = [
            f"Open item: {str(item.get('ask')).strip()}"
            for item in structured_watch.get("asks", [])
            if item.get("ask")
        ]
        open_items.extend(
            f"Open item: {str(item.get('deadline')).strip()}"
        ...
        if not open_items:
            open_items = [
                f"Open item: {point}"
                for point in self._next_actions("meeting_prep", ranked_threads)[:3]
            ]
        return open_items[:4]
    ...
    def _meeting_desired_outcomes(self, event_payload: Dict[str, Any], ranked_threads: List[Dict[str, Any]]) -> List[str]:
        ...
        if notes:
            outcomes.append(f"Desired outcome: make the decision implied by the prep notes: {notes}")
        for ask in asks[:2]:
            outcomes.append(f"Desired outcome: confirm the next step and owner for {ask.rstrip('.')}.")
        if deadlines:
            outcomes.append(f"Desired outcome: leave with follow-up timing locked against {deadlines[0].rstrip('.')}.")
        if not outcomes:
            outcomes = [
    ...
    def _email_suggested_actions(
        ...
        if action_required:
            ...
            actions.append(f"CEO: reply or delegate {subject} today.")
        if asks:
            actions.append(f"Delegate to the right owner: {asks[0].rstrip('.')} today.")
        if deadlines:
            actions.append(f"Lock ownership against {deadlines[0].rstrip('.')} before end of day.")
        if waiting:
            subject = str(waiting[0].get("subject") or "the blocked thread")
            actions.append(f"Nudge or escalate {subject} if no reply arrives by tomorrow morning.")
        if documents:
            actions.append(f"Use {documents[0].rstrip('.')} as the supporting document for the response.")
        if not actions:
            actions = [
    ...
    def _build_briefing_action_offers(
        ...
        # E1: Company state signal offers — fire even when there are no high-urgency threads.
        # Ordering: customer/deal context → delegation offer first; finance context → runway offer first.
        cs = company_state or {}
        ...
        elif isinstance(runway, (int, float)) and runway < 12:
            offers.append({
        ...
        if at_risk_initiatives and not offers and not is_customer_focus:
            initiative = at_risk_initiatives[0]
            name = initiative.get("name", "the at-risk initiative")
            owner = initiative.get("owner", "the responsible owner")
            offers.append({
        ...
        if offers:
            return offers[:1]  # Surface at most one action offer to avoid overwhelming the UI

        # Find the most urgent actionable thread
        high_urgency = [
    ...
    def _derive_trust(
        ...
        if workflow_type != "calendar_briefing":
            if len(actionable_threads) >= 2:
                ...
                missing_context.append("No actionable inbox threads were available.")
        ...
        if meeting_count >= 2:
            ...
            missing_context.append("Calendar evidence was limited.")
        ...
        if context_source_count >= 3:
            ...
            missing_context.append("The answer relies on a narrow evidence base.")
        ...
        if placed_candidate_count >= 3:
            ...
            missing_context.append("Planning candidates could not be cleanly placed into slots.")
        ...
        if top_thread and (top_thread.get("suppressed") or top_thread.get("category") == "promotional"):
            evidence_score -= 0.22
            missing_context.append("Top inbox evidence was suppressed or promotional.")
        ...
        if planning_context.get("mode") == "compound_plan":
            ...
            if len(completed_steps) >= 3:
                ...
                missing_context.append("Compound planning evidence was only partially assembled.")
        ...
        if sparse_guidance:
            evidence_score -= 0.08
            missing_context.append("The plan fell back to sparse guidance because the weekly evidence set was weak.")
        ...
        if len(actionable_threads) == 0 and workflow_type != "calendar_briefing":
            evidence_reasons.append("No actionable inbox threads were available.")
        if meeting_count == 0 and workflow_type in {"calendar_briefing", "schedule_planning", "morning_brief", "weekly_recap", "meeting_prep"}:
            evidence_reasons.append("Calendar evidence was limited.")
        if context_source_count <= 1:
            evidence_reasons.append("The briefing relies on a narrow set of supporting context.")
        if sparse_guidance:
            evidence_reasons.append("The planner fell back to sparse guidance.")
        ...
        action_offers = self._build_briefing_action_offers(
            ...
            ranked_threads=ranked_threads,
            task_input=task_input,
            company_state=prepared_context.get("company_state") or {},
        )
        return BriefTrust(
            ...
            question_options=action_offers,
        )
    ...
    def _schedule_follow_ups(
        ...
        *,
        ...
        items = [str(ask).strip() for ask in asks[:2] if str(ask).strip()]
        corpus = " ".join(
            [
                *(str(event.get("title") or "") for event in (event_payload.get("upcoming_events", []) or [])),
                *(str(item.get("document") or "") for item in ((event_payload.get("structured_watch", {}) or {}).get("implied_docs", []) or [])),
                *(str(thread.get("subject") or "") for thread in ranked_threads[:4]),
            ]
        ).lower()
        if "board packet" in corpus and not any("board packet" in item.lower() for item in items):
            items.append("Finalize board packet narrative.")
        if detailed:
        ...
        if not items:
            items = self._next_actions("schedule_planning", ranked_threads)
        return items[:3]
    ...
    def _structured_calendar(
        ...
        follow_ups = self._next_actions(workflow_type, [])
    ...
    def _apply_presentation_metadata(self, payload: BriefPayload, *, event_payload: Dict[str, Any], workflow_type: str) -> BriefPayload:
        ...
        if workflow_type == "schedule_planning":
            ...
            payload.presentation = BriefPresentation(
                ...
                variant=self._presentation_variant(workflow_type, event_payload=event_payload),
                preamble=preamble,
                summary=payload.answer.summary,
                priorities=[],
                recommended_actions=[],
    ...
    def _presentation_variant(self, workflow_type: str, *, event_payload: Dict[str, Any] | None = None) -> Optional[str]:
        if workflow_type == "schedule_planning":
            planning_context = ((event_payload or {}).get("planning_context") or {})
            time_horizon = planning_context.get("time_horizon", "unspecified")
            return "week_timeline" if time_horizon in {"this_week", "next_week"} else "timeline"
