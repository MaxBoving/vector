...
class ExplainerAgent(BaseAgent):
    ...
    def _generate_explanation_payload(
        ...
        if completion:
            try:
                payload = ExplanationPayload(**completion)
                return self._attach_semantic_followups(payload, task_input, retrieval, attachments)
            except Exception:
                pass
        return self._attach_semantic_followups(self._fallback_payload(task_input, retrieval, attachments), task_input, retrieval, attachments)
    ...
    def _semantic_context_for_explanation(
        self,
        *,
        task_input: str,
        ...
        title = payload.answer.title or task_input
        sections = [
            {"label": section.label, "content": section.content, "items": section.items}
            for section in payload.answer.sections
        ]
        source_ids = [str(item.get("source_id") or "") for item in payload.sources if isinstance(item, dict) and item.get("source_id")]
        return build_semantic_context(
            ...
            evidence_state="sparse" if not retrieval else "mixed",
            missing_context=payload.trust.missing_context,
            workflow_type="document_explanation",
            response_type="explanation",
            topic_hint=title,
            families_hint=["document"],
            source_ids_hint=source_ids,
            importance_hint=80.0 if retrieval else 60.0,
        )

    def _attach_semantic_followups(
        self,
        payload: ExplanationPayload,
        task_input: str,
        retrieval: List[Dict[str, Any]],
        ...
        semantic_context = self._semantic_context_for_explanation(
        ...
        payload.trust.question_options = build_semantic_question_options(semantic_context)
        return payload
