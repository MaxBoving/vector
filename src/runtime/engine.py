...
from src.workflows.workflow_contracts import workflow_presentation_mode, workflow_presentation_variant
...
class RuntimeEngine:
    ...
    def _build_clarification_response(
        ...
        *,
        ...
        return AssistantMessageResponse(
            ...
            trust=TrustMetadata(confidence="low", confidence_score=0.0, question_options=trust_options),
            sources=[],
            artifacts=[],
    ...
    def build_runner_clarification_response(
        ...
        *,
        ...
        trust_options = [
            {
                "question": clarification.question or preamble,
                "options": [
                    {
                        "label": str(option.get("label") or "Choose"),
                        "value": str(option.get("value") or "").strip(),
        ...
        response = AssistantMessageResponse(
            ...
            trust=TrustMetadata(
                ...
                question_options=trust_options,
            ),
            sources=[],
            artifacts=[],
            presentation=MessagePresentation(
    ...
    def _execute_actions(
        ...
        tool_context = self._build_tool_context(workflow_state, current_user)
            ...
            if write_artifact_action is not None:
                ...
                if result.success:
                    ...
                    record_world_event(
                        ...
                        description=f"Artifact written for stage {write_artifact_action.artifact_stage}.",
                        source_ids=[str(workflow_state.interaction_id or 0)],
                        payload={
                            "stage": write_artifact_action.artifact_stage,
                            "filename": write_artifact_action.filename,
                            "metadata": write_artifact_action.metadata.model_dump(mode="json"),
                            "interaction_id": workflow_state.interaction_id,
                        },
                    )
                ...
                record_world_event(
                    ...
                    domain="artifact",
                    event_type="artifact_written",
                    description=f"Generated artifact created for stage {artifact_action.artifact_stage}.",
                    source_ids=[str(workflow_state.interaction_id or 0)],
                    payload={
                        "stage": artifact_action.artifact_stage,
                        "filename": artifact_action.filename,
                        "tool_name": artifact_action.tool_name,
                        "interaction_id": workflow_state.interaction_id,
                    },
                )
    ...
    def _execute_context_stage(
        ...
        *,
        ...
        if workflow_state.metadata.get("request_plan"):
            aggregate_context.setdefault("request_plan", workflow_state.metadata["request_plan"])
        ...
        actions = build_context_stage_actions(
            ...
            payload.message,
            {
                ...
                "attachments": workflow_state.metadata.get("attachments"),
            },
        )
        if actions:
    ...
    def _build_tool_context(self, workflow_state: WorkflowState, current_user: User) -> ToolContext:
        return ToolContext(
            workflow_id=workflow_state.workflow_id,
            interaction_id=workflow_state.interaction_id,
            ceo_id=current_user.ceo_id,
    ...
    def _normalize_trust_payload(self, raw_trust: dict[str, Any] | None) -> dict[str, Any]:
        ...
        semantic_context = trust.get("semantic_context")
        if isinstance(semantic_context, dict):
            trust["semantic_context"] = semantic_context
        elif semantic_context is None:
            trust["semantic_context"] = None
    ...
    def _normalize_weekly_plan_payload(self, raw_weekly_plan: Any) -> dict[str, Any] | None:
        ...
        return WeeklyPlanPresentation(
            planning_window=WeeklyPlanWindow(
                ...
                span_days=int(planning_window.get("span_days")) if isinstance(planning_window, dict) and planning_window.get("span_days") is not None else None,
    ...
    def _normalize_spreadsheet_payload(self, raw_spreadsheet: Any) -> dict[str, Any] | None:
        ...
        for index, raw_column in enumerate(raw_spreadsheet.get("columns") or []):
            if not isinstance(raw_column, dict):
                continue
            key = str(raw_column.get("key") or "").strip() or f"column_{index + 1}"
            label = str(raw_column.get("label") or "").strip() or key.replace("_", " ").title()
            align = str(raw_column.get("align") or "").strip().lower() or None
            columns.append(
                SpreadsheetColumn(
                    key=key,
                    label=label,
                    width=_parse_int(raw_column.get("width")),
                    align=align,
                )
            )

        rows: list[SpreadsheetRow] = []
        for raw_row in raw_spreadsheet.get("rows") or []:
            ...
            for raw_cell in raw_row.get("cells") or []:
                if isinstance(raw_cell, dict):
                    cells.append(
                        SpreadsheetCell(
                            value=str(raw_cell.get("value")) if raw_cell.get("value") is not None else None,
                            kind=str(raw_cell.get("kind") or "").strip().lower() or None,
                            align=str(raw_cell.get("align") or "").strip().lower() or None,
                        )
                    )
                else:
                    cells.append(SpreadsheetCell(value=str(raw_cell)))
            rows.append(
                SpreadsheetRow(
                    label=str(raw_row.get("label")).strip() if raw_row.get("label") else None,
                    cells=cells,
                )
            )

        return SpreadsheetPresentation(
            title=str(raw_spreadsheet.get("title")).strip() if raw_spreadsheet.get("title") else None,
            subtitle=str(raw_spreadsheet.get("subtitle")).strip() if raw_spreadsheet.get("subtitle") else None,
            columns=columns,
            rows=rows,
            frozen_columns=_parse_int(raw_spreadsheet.get("frozen_columns")) or 0,
            total_rows=_parse_int(raw_spreadsheet.get("total_rows")),
            source_artifact_id=str(raw_spreadsheet.get("source_artifact_id")).strip() if raw_spreadsheet.get("source_artifact_id") else None,
        ).model_dump(exclude_none=True)
    ...
    def _normalize_presentation_payload(self, raw_presentation: Any) -> dict[str, Any] | None:
        ...
        spreadsheet = self._normalize_spreadsheet_payload(raw_presentation.get("spreadsheet"))
        mode = str(raw_presentation.get("mode") or "").strip().lower()
        if not mode and spreadsheet:
            mode = "spreadsheet"
        return MessagePresentation(
            mode=mode if mode in {"brief", "report", "schedule", "decision", "draft", "finance", "artifact", "media", "calendar", "canvas", "spreadsheet"} else None,
            ...
            spreadsheet=SpreadsheetPresentation(**spreadsheet) if spreadsheet else None,
        ).model_dump(exclude_none=True)
    ...
    def _handle_failure_transition(
        ...
        *,
        ...
        failure_response = AssistantMessageResponse(
            ...
            artifacts=[],
            presentation=MessagePresentation(
                mode=workflow_presentation_mode(definition.workflow_type),
                variant=workflow_presentation_variant(definition.workflow_type),
                summary=definition.metadata.get("failure_summary", "The system could not complete this request."),
            ),
            metadata={
                "interaction_id": interaction.id,
