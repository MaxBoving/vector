...
def build_financial_analysis_task(
    *,
    ...
    report_intent: Dict[str, Any] | None = None,
) -> FinancialAnalysisTask | None:
    intent_state = intent_state or {}
    report_intent = report_intent or {}
    ...
    task_topic = str(
        intent_state.get("task_topic")
        or (working or {}).get("task_topic")
        or report_intent.get("task_topic")
        or ""
    ).strip()
    ...
    if task_topic == "pricing_response":
        return FinancialAnalysisTask(
            ...
            deliverable_mode="execution_bundle"
            if str((report_intent.get("deliverable") or {}).get("kind") or "") in {"execution_bundle", "draft", "send"}
            else "report",
    ...
    if task_topic == "cost_containment":
        return FinancialAnalysisTask(
    ...
    if task_topic == "variance_analysis":
        return FinancialAnalysisTask(
            task_type="variance_analysis",
            decision_goal="Explain the period-over-period variance and what changed.",
            analysis_scope="Actual versus plan, drivers, and decision implications.",
            time_horizon=time_horizon,
            entities=entities,
            metrics_requested=["variance", "actual", "plan", "drivers"],
            deliverable_mode="report",
            required_sections=["Variance Summary", "Key Drivers", "Decision Implications"],
            required_tables=["variance_table"],
            required_actions=["name the main drivers"],
            constraints=[],
            rationale="The structured task topic indicates variance analysis.",
        )

    if task_topic == "scenario_model":
        return FinancialAnalysisTask(
            task_type="scenario_model",
            decision_goal="Compare plausible scenarios and the decision implications.",
            analysis_scope="Base case, downside, upside, and the assumptions behind each.",
            time_horizon=time_horizon,
            entities=entities,
            metrics_requested=["revenue", "burn", "runway", "margin"],
            deliverable_mode="report",
            required_sections=["Base Case", "Downside Case", "Upside Case", "Assumptions"],
            required_tables=["scenario_table"],
            required_actions=["state assumptions", "name decision points"],
            constraints=[],
            rationale="The structured task topic indicates scenario modeling.",
        )

    if finance_template or bool(report_intent.get("is_finance")):
        return FinancialAnalysisTask(
    ...
    if bool(report_intent.get("is_escalation")) or task_topic == "operating_review":
        return FinancialAnalysisTask(
            task_type="operating_review",
            decision_goal="Structure the operational review into decision-ready sections.",
            analysis_scope="Operating issues, key metrics, risks, and recommended actions.",
            time_horizon=time_horizon,
            entities=entities,
            metrics_requested=["key_metrics", "risks", "actions"],
            deliverable_mode="report",
            required_sections=["Key Findings", "Decision Implications", "Recommended Actions"],
            required_tables=["operating_review"],
            required_actions=["name the main actions"],
            constraints=[],
            rationale="The report intent indicates a structured operating review.",
        )
